import { AiPlanCategory, AiPlanRequest, AiPlanResponse, AiPlanHabit, AiPlanDay } from './ai.types';

interface CategoryTemplate {
  habits: AiPlanHabit[];
  tasksByDay: string[][]; // index 0 = day 1
}

const RU: Record<AiPlanCategory, CategoryTemplate> = {
  sport: {
    habits: [
...
const EN: Record<AiPlanCategory, CategoryTemplate> = {
  sport: {
    habits: [
      { title: 'Morning warm-up', description: '5 minutes right after waking' },
      { title: 'Daytime activity', description: '10–20 minutes of walking or a short workout' },
  ...
  study: {
    habits: [
      { title: 'Morning review', description: 'A short review of yesterday' },
...
export function localStubPlan(req: AiPlanRequest): AiPlanResponse {
  const tpl = (req.language === 'en' ? EN : RU)[req.category];
  const horizon = Math.max(1, Math.min(req.horizonDays, 30));
  const schedule: AiPlanDay[] = [];
  for (let i = 0; i < horizon; i++) {
    const tasks = tpl.tasksByDay[i % tpl.tasksByDay.length];
    schedule.push({ day: i + 1, tasks: tasks.slice(0, 3) });
  }
  return {
    provider: 'stub',
